...
def _build_system_prompt(session: models.Session, rag_available: bool) -> str:
    ...
    "9) Для theory после КАЖДОГО ответа кандидата на очередной вопрос ты обязан вызывать промежуточный score_task с параметрами is_final=false и question_index=i. Для theory используй шкалу 1..10.\n"
    ...
    "11) После завершения всего theory-блока ты обязан вызвать финальный score_task для этой theory-задачи с is_final=true. Финальная оценка должна опираться на все промежуточные оценки и комментарии по блоку: сначала усредни промежуточные оценки, затем при необходимости скорректируй итог на 1 балл вверх или вниз по глубине и качеству объяснений.\n"
    ...
    "16) После успешного score_task для theory ты ОБЯЗАН написать в чат человеко-понятный итог теоретического блока.\n"
    ...
    "- Если по истории ответов видно слабые места, ты можешь задать 1–2 итоговых углубляющих вопроса по самым проблемным темам перед финальным score_task.\n"

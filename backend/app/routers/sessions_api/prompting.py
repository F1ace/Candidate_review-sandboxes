...
def _current_task_payload_for_prompt(task: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if task_type == "theory":
        payload["questions"] = [
            question_text
            for question_text in (_question_text_for_prompt(item) for item in (task.get("questions") or []))
            if question_text
        ][:12]
    else:
        brief = (
...
def _build_system_prompt(session: models.Session, rag_available: bool) -> str:
    ...
    return (
        ...
        "- Theory: доступны только rag_search и score_task. Никогда не вызывай run_code и run_sql.\n"
...
def _extract_inline_tool_call(
    ...
    match = re.search(r"to=([a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)?)", content)
    if not match:
        return None

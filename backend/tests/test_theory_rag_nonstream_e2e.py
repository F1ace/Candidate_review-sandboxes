...
def test_nonstream_theory_flow_uses_contract_repair_and_final_points(
    client,
    db_session,
        ...
        if tools:
            ...
            if "Все промежуточные оценки theory-блока уже сохранены" in system_text:
                args = {
                    ...
                    "points": 5.0,
                    "comment": (
                        "Кандидат уверенно описал основу темы и не ошибся в трактовке POST. "
                        "До сильного итогового результата не хватило более строгого объяснения границ идемпотентности."
                    ),
                    "comments": [
                        (
                            "Кандидат корректно объяснил базовый смысл идемпотентности и верно отметил, что POST обычно не считается "
                            "идемпотентным, но не раскрыл, где заканчивается свойство метода и начинается поведение конкретного API."
                        )
                    ],
                    "is_final": True,
                    "question_index": None,
                ...
                "choices": [
                    {
                        "message": {
                            "role": "assistant",
                            "content": "Теоретический блок завершён. Итоговая оценка: 8/10.",
                        }
                    }
                ]
            }
            ...
            "choices": [
                {
                    "message": {
                        ...
                        "content": (
                            ...
                            "Теоретический блок завершён. По ответу видно базовое понимание темы, но детализация местами осталась поверхностной.\n\n"
                            "- **Идемпотентность и POST:** Кандидат корректно объяснил базовый смысл идемпотентности и верно отметил, "
                            "что POST обычно не считается идемпотентным, но не раскрыл, где заканчивается свойство метода и начинается поведение конкретного API.\n\n"
                            "**Сильные стороны:**\n"
                            "- Уверенно знает базовое определение идемпотентности.\n"
                            "- Не путает общую семантику POST.\n\n"
                            "**Зоны роста:**\n"
                            "- Точнее объяснять границы между методом HTTP и поведением endpoint.\n"
                            "- Добавлять больше технической конкретики в развёрнутый ответ.\n\n"
                            "**Итоговая оценка по теоретическому блоку:** 5/10."
                        ),
                    }
                }
            ]
    ...
    assert "**Итоговая оценка по теоретическому блоку:** 5/10." in final_text
...
def test_nonstream_theory_rag_final_points_are_capped_by_intermediate_avg(
    ...
    def fake_chat(messages, tools=None, tool_choice=None, temperature=0.2):
        ...
        if tools:
            ...
            assert tool_names == {"score_task"}
            if "Все промежуточные оценки theory-блока уже сохранены" in system_text:
                assert "avg_points уже рассчитан системой и равен 2" in system_text
                args = {
                    "task_id": "T-DOCS",
                    "points": 8.0,
                    "comment": (
                        "Кандидат дал слабый итог по теме и допустил несколько базовых ошибок. "
                        "При этом в ответе всё же была попытка рассуждать, поэтому итоговый уровень можно кратко описать как низкий."
                    ),
                    "comments": [
                        (
                            "Кандидат перепутал смысл идемпотентности и ошибочно назвал POST всегда идемпотентным, "
                            "поэтому ответ остался слабым и неточным."
                        )
                    ],
                    "is_final": True,
                    "question_index": None,
                ...
                args = {
                    ...
                    "points": 2.0,
                    "comment": (
                        "Ответ противоречит документам сценария и содержит существенные ошибки в базовом определении. "
                        "Кандидат неверно трактует идемпотентность и ошибочно описывает поведение POST."
                    ),
                    "is_final": False,
                    "question_index": 1,
                }
            ...
            "choices": [
                {
                    "message": {
                        ...
                        "content": (
                            "**Итоги теоретической части**\n\n"
                            "Теоретический блок завершён. В ответе допущены существенные ошибки в базовых понятиях.\n\n"
                            "- **Идемпотентность и POST:** Кандидат перепутал смысл идемпотентности и ошибочно назвал POST "
                            "всегда идемпотентным, поэтому ответ остался слабым и неточным.\n\n"
                            "**Сильные стороны:**\n"
                            "- Пытался дать развернутый ответ по теме.\n\n"
                            "**Зоны роста:**\n"
                            "- Разобраться в базовом определении идемпотентности.\n"
                            "- Точнее понимать стандартную семантику POST.\n\n"
    ...
    assert "**Итоговая оценка по теоретическому блоку:** 2/10." in final_text
    ...
    assert len(scores) == 2
    assert scores[0].is_final is False
    assert scores[0].points == 2.0
    assert scores[1].is_final is True
    assert scores[1].points == 2.0
    ...
    assert len(tool_messages) == 3
    assert "'avg_points': 2" in (tool_messages[-1].text or "")
    assert len(state["tool_sequences"]) == 3
    assert state["summary_calls"] == 1
    assert embeddings_backend.document_calls
    assert embeddings_backend.query_calls
...
def test_nonstream_theory_flow_coerces_missing_is_final_for_intermediate_score(
    client,
    db_session,
    theory_scenario_factory,
    monkeypatch,
...
def test_nonstream_theory_flow_repairs_comments_only_final_summary(
    client,
    db_session,
    theory_scenario_factory,
    monkeypatch,
...
def test_nonstream_theory_rag_flow_clamps_intermediate_points_below_one(
    client,
    db_session,
    theory_scenario_factory,
    monkeypatch,
...
def test_nonstream_theory_rag_final_message_cleans_comment_table_garbage(
    client,
    db_session,
    theory_scenario_factory,
    monkeypatch,
...
def test_nonstream_theory_rag_final_message_removes_duplicate_alternative_comment_header(
    client,
    db_session,
    theory_scenario_factory,
    monkeypatch,
...
def test_nonstream_theory_rag_final_message_removes_prefix_table_garbage_before_comments(
    client,
    db_session,
    theory_scenario_factory,
    monkeypatch,
